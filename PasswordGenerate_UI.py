...
def generate(length,**kwargs):
    ...
    if not any(kwargs.values()):
        return 'Please provide the at least one password requirement'
    
    for key in kwargs:
        if kwargs[key] and CHAR_MAP.get(key):
            chars += CHAR_MAP[key]
...
class Model:
    def __init__(self):
        ...
        self.number=BooleanVar(value=True)
        self.special_character=BooleanVar(value=True)
...
class View:
    def __init__(self,main):
        ...
        self.lenght_display=CTkLabel(self.main,textvariable=self.model.length)
        ...
        self.number_checkbutton=CTkCheckBox(self.main,text='Numbers',variable=self.model.number)
        self.special_checkbutton=CTkCheckBox(self.main,text='Special Characters',variable=self.model.special_character)
        ...
        self.number_checkbutton.place(relx=0.4 ,rely=0.45  ,anchor='nw')
    ...
    def generate_password(self):
        self.model.password.set(
            generate(
                ...
                numbers=self.model.number.get(),
                special_character=self.model.special_character.get()
            )
        )
